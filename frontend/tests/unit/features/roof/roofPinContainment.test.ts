...
import { isPointInsidePolygon } from "../../../../src/features/roof/roofUtils";
...
describe("isPointInsidePolygon", () => {
  ...
  it("handles a turned outline, not just a square-to-north one", () => {
    // A clicked outline follows the building, not the compass, so a test that
    // only ever sees axis-aligned corners would not exercise the real case.
    const diamond = [
